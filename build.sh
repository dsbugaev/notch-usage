...
# Sign with a stable local self-signed identity so keychain "Always Allow"
# survives rebuilds (ad-hoc signing changes identity every build and re-prompts).
# Any failure below degrades to ad-hoc signing instead of aborting the build.
IDENTITY="NotchUsage Signing"

create_identity() {
  local tmpd
  tmpd=$(mktemp -d) || return 1
  # /usr/bin/openssl (LibreSSL): its PKCS12 defaults are accepted by
  # `security import`, unlike Homebrew OpenSSL 3.x default ciphers
...
if ! codesign --force -s "$IDENTITY" "$APP" 2>/dev/null; then
  echo "warning: stable identity unavailable, signing ad-hoc (keychain will re-prompt after rebuilds)"
